...
TEACHER_CONFIG_PATH = Path(__file__).resolve().parent / "configs" / "hybrid_teachers.yaml"


def _load_hybrid_defaults() -> dict:
    try:
        with TEACHER_CONFIG_PATH.open("r", encoding="utf-8") as fh:
            return yaml.safe_load(fh) or {}
    except FileNotFoundError:
        return {}


_HYBRID_DEFAULTS = _load_hybrid_defaults()
_LM_TEACHER_DEFAULT = _HYBRID_DEFAULTS.get("lm", {}).get("teacher_id", "meta-llama/Llama-3.2-3B")
_MC_TEACHER_DEFAULT = _HYBRID_DEFAULTS.get("math_tool", {}).get("teacher_id", "meta-llama/Llama-3.1-8B")
_MATH_LOGITS_DEFAULT = _HYBRID_DEFAULTS.get(
    "math_tool", {}
).get("logits_dir", "gs://liquid-llm-bucket-2/teacher/llama-3.1-8b/logits/math/")
_CODE_LOGITS_DEFAULT = _HYBRID_DEFAULTS.get(
    "code", {}
).get("logits_dir", "gs://liquid-llm-bucket-2/teacher/llama-3.1-8b/logits/code/")
...
DEFAULTS = {
    ...
    "teacher_id": _LM_TEACHER_DEFAULT,
    ...
    "hybrid": True,
    "lm_teacher_id": _LM_TEACHER_DEFAULT,
    "mc_teacher_id": _MC_TEACHER_DEFAULT,
    "math_logits_dir": _MATH_LOGITS_DEFAULT,
    "code_logits_dir": _CODE_LOGITS_DEFAULT,
    "hybrid_strict": False,
}
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument("--hybrid", type=_str_to_bool, default=DEFAULTS["hybrid"])
    parser.add_argument("--hybrid-strict", type=_str_to_bool, default=DEFAULTS["hybrid_strict"])
    parser.add_argument("--lm-teacher-id", default=DEFAULTS["lm_teacher_id"])
    parser.add_argument("--mc-teacher-id", default=DEFAULTS["mc_teacher_id"])
    parser.add_argument("--math-logits-dir", default=DEFAULTS["math_logits_dir"])
    parser.add_argument("--code-logits-dir", default=DEFAULTS["code_logits_dir"])
...
def _log_startup_banner(
    ...
    logger.info(
        "teacher_mode=%s | hybrid=%s | tokenizer_id=%s",
        teacher_mode,
        hybrid,
        teacher_id,
    )
    logger.info(
        "lm_teacher=%s | mc_teacher=%s | math_logits_dir=%s | code_logits_dir=%s",
        lm_teacher_id,
        mc_teacher_id,
        math_logits_dir,
        code_logits_dir,
    ...
    logger.info(
        "kd_temp=%.2f | alpha_start=%.2f | alpha_end=%.2f | anneal_pct=%.2f",
        kd_temperature,
        alpha_start,
        alpha_end,
        anneal_pct,
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    if teacher_mode_arg not in {"online", "precompute"}:
        raise ValueError(f"Invalid teacher_mode: {teacher_mode_arg}")
    hybrid_enabled = bool(args.hybrid)
    hybrid_strict = bool(args.hybrid_strict)
    if not hybrid_enabled and teacher_mode_arg == "precompute" and not args.teacher_logits_dir:
    ...
    tokenizer_id = args.lm_teacher_id if hybrid_enabled else args.teacher_id
    tokenizer = data.build_tokenizer(tokenizer_id)
    manifest_entries, manifest_snapshot = data.read_manifest(args.dataset_manifest, datasets_cfg)
    teacher_logits_dir = (
        args.teacher_logits_dir if (teacher_mode_arg == "precompute" and not hybrid_enabled) else None
    )
    ...
    if args.val_manifest:
        ...
        val_dataset = data.ManifestDataset(
            ...
            seq_len=args.seq_len,
            tool_use_ratio=0.0,
            teacher_mode=effective_teacher_mode if hybrid_enabled else "precompute",
            teacher_logits_dir=teacher_logits_dir,
            hybrid=hybrid_enabled,
            math_logits_dir=math_logits_dir,
            code_logits_dir=code_logits_dir,
    ...
    if hybrid_enabled:
        lm_sample_count = dataset.sample_type_counts.get("lm", 0)
        if lm_sample_count > 0:
            lm_teacher = TeacherWrapper(TeacherConfig(model_id=args.lm_teacher_id))
        else:
            logger.warning("Hybrid mode enabled but no lm samples detected; skipping LM teacher initialization")
    elif teacher_mode_arg == "online":
        lm_teacher = TeacherWrapper(TeacherConfig(model_id=args.teacher_id))
    trainer = Trainer(
        model=model,
