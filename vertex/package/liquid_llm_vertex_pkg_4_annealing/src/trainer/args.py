...
def get_parser():
    ...
    p.add_argument('--alpha_schedule', type=str, default=None,
                   help='Comma separated schedule step:value entries for KD alpha annealing')
    p.add_argument('--temp_schedule', type=str, default=None,
                   help='Comma separated schedule step:value entries for KD temperature annealing')
...
def parse_args(argv=None):
    ...
    def parse_schedule(value):
        ...
        if isinstance(value, (list, tuple)):
            entries = [(int(step), float(val)) for step, val in value]
            entries.sort(key=lambda x: x[0])
            return entries
        if isinstance(value, dict):
            entries = [(int(step), float(val)) for step, val in value.items()]
            entries.sort(key=lambda x: x[0])
            return entries
        entries = []
    ...
    cfg = {
        ...
        'T': o('T', merged.get('T')),
        'alpha_schedule_raw': o('alpha_schedule', merged.get('alpha_schedule')),
        'temp_schedule_raw': o('temp_schedule', merged.get('temp_schedule')),
        'alpha_schedule': parse_schedule(o('alpha_schedule', merged.get('alpha_schedule'))),
        'temp_schedule': parse_schedule(o('temp_schedule', merged.get('temp_schedule'))),
