...
class StudentLM(nn.Module):
    ...
    def resize_positional_embeddings(self, new_max_positions: int) -> bool:
        """Resize positional embeddings to accommodate a new maximum context length."""
        new_max_positions = int(new_max_positions)
        if new_max_positions <= 0:
            return False
        old_max = int(getattr(self.pos, "num_embeddings", 0))
        if new_max_positions == old_max and self.pos_ids.size(-1) == new_max_positions:
            return False

        weight = self.pos.weight.data
        device = weight.device
        dtype = weight.dtype
        if new_max_positions <= old_max and old_max > 0:
            new_weight = weight[:new_max_positions].clone()
        else:
            src = weight.unsqueeze(0).transpose(1, 2)
            new_weight = (
                F.interpolate(src, size=new_max_positions, mode="linear", align_corners=True)
                .transpose(1, 2)
                .squeeze(0)
            ).to(dtype=dtype)

        embed = nn.Embedding(new_max_positions, weight.size(1)).to(device=device, dtype=dtype)
        embed.weight.data.copy_(new_weight)
        self.pos = embed
        self.register_buffer(
            "pos_ids", torch.arange(0, new_max_positions, device=device).unsqueeze(0), persistent=False
        )
        return True
