...
def _apply_schedule(state, step, key, sched_key):
    """Pop schedule entries whose trigger step has been reached."""
    sched = state.get(sched_key)
    if not sched:
        return False
    changed = False
    while sched and step >= sched[0][0]:
        _, val = sched.pop(0)
        state[key] = val
        changed = True
    return changed
...
def _save_ckpt(ckptio, state, step, filename, log):
    ...
    if extra_builder is not None:
        extra_payload = extra_builder(step)
        if extra_payload:
            trainer_state = extra_payload.get("trainer_state")
            if trainer_state:
                state_dict["trainer_state"] = trainer_state
            meta = extra_payload.get("meta")
            if meta is not None:
                state_dict["meta"] = meta
    local, uri = ckptio(
        state_dict,
        state["local_outdir"],
    ...
    try:
        ...
        size_bytes = 0
    if meta:
        log.info("[ckpt-meta] step=%s filename=%s meta=%s", step, filename, json.dumps(meta, sort_keys=True))
...
def train_loop(state):
    ...
    eval_ctx_loaders = state.get("eval_ctx_loaders") or {}
    grad_accum = max(1, int(state.get("grad_accum", 1)))
    state["grad_accum"] = grad_accum
    ...
    while step < total_steps:
        if accum_counter == 0:
            ...
            pending_step = step + 1
            alpha_changed = _apply_schedule(state, pending_step, "kd_alpha", "kd_alpha_schedule")
            temp_changed = _apply_schedule(state, pending_step, "kd_temperature", "kd_temperature_schedule")
            if alpha_changed or temp_changed or pending_step == 1:
                maybe_log_kd_params(pending_step)
        ...
        total_loss_value = float(combined_loss.detach())
        if (
        ...
        loss_for_backward = combined_loss / float(grad_accum)
