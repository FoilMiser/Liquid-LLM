...
def _refresh_kd_schedules(state, phase_step: int) -> bool:
...
def train_loop(state):
    ...
    def maybe_log_kd_params(step_idx: int, phase_step_idx: int):
    ...
    maybe_log_kd_params(step, phase_step)
    ...
    while step < total_steps:
        if accum_counter == 0:
            ...
            schedule_changed = _refresh_kd_schedules(state, phase_step)
            kd_alpha = float(state.get("kd_alpha", kd_alpha))
            kd_temperature = float(state.get("kd_temperature", kd_temperature))
            if schedule_changed:
                maybe_log_kd_params(step, phase_step)
