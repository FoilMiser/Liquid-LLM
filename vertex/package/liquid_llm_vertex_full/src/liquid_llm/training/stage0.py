...
def run_training(
    ...
    if kd_alpha and kd_alpha > 0:
        log.info(
            f"Loading teacher model {teacher_name} for knowledge distillation (alpha={kd_alpha}, T={kd_temperature})"
        )
        teacher_dtype = None
        if precision in {"fp16", "bf16"} and torch.cuda.is_available():
            teacher_dtype = torch.float16 if precision == "fp16" else torch.bfloat16
    ...
    # ---------------------------------------------------------------------
    # Baseline evaluation (teacher/student) prior to training
    # ---------------------------------------------------------------------
    try:
        student_metrics = evaluate(student, val_dl, device=device, precision=precision)
        log.info(
            "[init/student] val_loss=%.4f ppl=%.2f tokens=%d",
            student_metrics["val_loss"],
            student_metrics["val_ppl"],
            student_metrics["val_tokens"],
        )
    except Exception as exc:
        log.warning(f"Initial student evaluation failed: {exc}")
        student_metrics = None

    teacher_metrics = None
    if teacher is not None:
        try:
            teacher_metrics = evaluate(teacher, val_dl, device=device, precision=precision)
            log.info(
                "[init/teacher] val_loss=%.4f ppl=%.2f tokens=%d",
                teacher_metrics["val_loss"],
                teacher_metrics["val_ppl"],
                teacher_metrics["val_tokens"],
            )
        except Exception as exc:
            log.warning(f"Initial teacher evaluation failed: {exc}")
